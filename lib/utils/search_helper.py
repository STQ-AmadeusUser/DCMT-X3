    ...
    for i in range(meas_times):
        with torch.no_grad():
            if not isinstance(input_data, tuple):
                if module._get_name() != 'Cls_Reg':
                    start = time.time()
                    output_data = module(input_data)
                    torch.cuda.synchronize()
                    if i >= 100:
                        latency.append(time.time() - start)
                else:  # the final module
                    output_data = None
                    start = time.time()
                    _, _ = module(input_data)
                    torch.cuda.synchronize()
                    if i >= 100:
                        latency.append(time.time() - start)
            else:
                input_data1, input_data2, input_data3 = input_data
                start = time.time()
                output_data = module(input_data1, input_data2, input_data3)
                torch.cuda.synchronize()
                if i >= 100:
                    latency.append(time.time() - start)
    print(np.mean(latency) * 1e3, 'ms')
