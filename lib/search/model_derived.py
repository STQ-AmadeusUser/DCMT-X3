...
import lib.utils.search_helper as searcher
from lib.models.backbone.backbone import Backbones
from lib.models.neck.neck import ShrinkChannel
from lib.models.match.match import CFMN, CNNPREFUSION, TRANSPREFUSION
...
class DCMT_Net(nn.Module):
    def __init__(self, net_config, config=None):
        """
        net_config=[[in_ch, out_ch], head_op, [stack_ops], num_stack_layers, stride]
        """
        super(DCMT_Net, self).__init__()
        self.config = config
        self.net_config = searcher.parse_net_config(net_config)

        backbone_cfg = self.net_config.pop(0)
        self.backbone = Backbones[backbone_cfg[1]](affine=True, track_running_stats=True)
        neck_cfg = self.net_config.pop(0)
        self.neck = ShrinkChannel(neck_cfg[0][0], neck_cfg[0][1], backbone_name=backbone_cfg[1], affine=True, track_running_stats=True)
        match_cfg = self.net_config.pop(0)
        if match_cfg[2] == 'CFMN':
            self.match = CFMN(match_cfg[0][0], match_cfg[0][1], match_cfg[0][2], match_cfg[1], affine=True, track_running_stats=True)
        elif match_cfg[2] == 'CNNPREFUSION':
            self.match = CNNPREFUSION(match_cfg[0][0], match_cfg[0][1], match_cfg[1], affine=True, track_running_stats=True)
        elif match_cfg[2] == 'TRANSPREFUSION':
            self.match = TRANSPREFUSION(match_cfg[0][0], match_cfg[0][1], match_cfg[1], affine=True, track_running_stats=True)
        else:
