...
class Dropped_Network(nn.Module):
    def __init__(self, super_model, alpha_head_index=None, alpha_stack_index=None, softmax_temp=1.):
        ...
        self.backbone = super_model.module.backbone
        self.neck = super_model.module.neck
        self.match = super_model.module.match
        self.collector = super_model.module.collector
        self.prediction = super_model.module.prediction

        # architecture parameters loading
        self.num_alpha_head_weights = len(super_model.module.alpha_head_weights)
        self.num_alpha_stack_weights = len(super_model.module.alpha_stack_weights)
        self.num_beta_weights = len(super_model.module.beta_weights)
        for alpha_idx in range(self.num_alpha_head_weights):
            self.register_parameter('alpha_head_weights_{}'.format(alpha_idx), super_model.module.alpha_head_weights[alpha_idx])
            self.register_parameter('alpha_stack_weights_{}'.format(alpha_idx), super_model.module.alpha_stack_weights[alpha_idx])
        for beta_idx in range(self.num_beta_weights):
            self.register_parameter('beta_weights_{}'.format(beta_idx), super_model.module.beta_weights[beta_idx])
        self.alpha_head_index = alpha_head_index if alpha_head_index is not None else super_model.module.alpha_head_index
        self.alpha_stack_index = alpha_stack_index if alpha_stack_index is not None else super_model.module.alpha_stack_index
        ...
        for i, block in enumerate(super_model.module.fuse):
    ...
    def forward(self, z, x, bbox):
        ...
        branch_weights = []
        for beta_idx in range(self.num_beta_weights):
            branch_weights.append(F.softmax(
                getattr(self, 'beta_weights_{}'.format(beta_idx)) / self.softmax_temp, dim=-1))
        ...
        for i in range(len(self.fuse) + 1):
            ...
            if i == len(self.fuse):
                block_data, block_sub_obj = self.collector(inputs, betas, self.sub_obj_list[2])

            else:
                block_data, block_sub_obj = self.fuse[i](inputs,
                                                         betas,
                                                         getattr(self, 'alpha_head_weights_{}'.format(i)),
                                                         getattr(self, 'alpha_stack_weights_{}'.format(i)),
        ...
        cls, reg = self.prediction(block_datas[-1])
        ...
        return cls, reg, net_sub_obj.expand(1)
    
    @property
    def arch_parameters(self):

        arch_params = nn.ParameterList()
        for idx in range(self.num_beta_weights):
            arch_params.extend(getattr(self, 'beta_weights_{}'.format(idx)))
        for idx in range(self.num_alpha_head_weights):
            arch_params.extend(getattr(self, 'alpha_head_weights_{}'.format(idx)))
        for idx in range(self.num_alpha_stack_weights):
            arch_params.extend(getattr(self, 'alpha_stack_weights_{}'.format(idx)))
        return arch_params
    ...
    def arch_alpha_params(self):
        ...
        for idx in range(self.num_alpha_head_weights):
            alpha_params.extend(getattr(self, 'alpha_head_weights_{}'.format(idx)))
        for idx in range(self.num_alpha_stack_weights):
            alpha_params.extend(getattr(self, 'alpha_stack_weights_{}'.format(idx)))
