...
class BaseArchGenerate(object):
    ...
    def derive_chs(self):  # https://blog.csdn.net/Flag_ing/article/details/123123660
        """
    ...
    def derive_ops(self, alpha, alpha_type):
        ...
        if alpha_type == 'head':
            op_type = self.config.SEARCH.PRIMITIVES_HEAD[alpha.index(max(alpha))]
        elif alpha_type == 'stack':
            op_type = self.config.SEARCH.PRIMITIVES_STACK[alpha.index(max(alpha))]
        else:
